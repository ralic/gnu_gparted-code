...
root            (hd0,3)<br>
kernel 		/live-hd/vmlinuz boot=live union=aufs noswap noprompt vga=788 ip=frommedia <font color="red">live-media-path=/live-hd bootfrom=/dev/hda4 toram=filesystem.squashfs</font><br>
...
menuentry "GParted live" {<br>
  set root=(hd0,4)<br>
  linux /live-hd/vmlinuz boot=live union=aufs noswap noprompt vga=788 ip=frommedia <font color="red">live-media-path=/live-hd bootfrom=/dev/hda4 toram=filesystem.squashfs</font><br>
  initrd /live-hd/initrd.img<br>
} <br>
<p>================================================<br>
   //NOTE// In grub2, (hd0,4) means the first harddrive and the 4th partition. It's different from that in grub version 1. Then run "update-grub2" to update your grub2 config. (Thanks to Louie Chen for providing this). <br><br>
       Besides, from GParted live version 0.4.8-7, you can use only the GParted live iso file in grub2 (Thanks to the patches files from <a href="http://grml.org" target=_blank>grml</a>). For example, put gparted-live-0.5.2-9.iso in dir /home/isos/, then make the grub2 custom file /etc/grub.d/40_custom like:<br>
<p>================================================<br>
    menuentry "Gparted live" {<br>
    set isofile="/home/isos/gparted-live-0.5.2-9.iso"<br>

    loopback loop $isofile<br>
    linux (loop)/live/vmlinuz boot=live union=aufs noswap noprompt vga=788 ip=frommedia <font color="red">toram=filesystem.squashfs findiso=$isofile</font><br>
...
<p>================================================<br>
   ...
   <font color="red">Remember to check parameters in syslinux/syslinux.cfg from the zip file, copy them to here. It might be different from here, say vmlinuz path maybe different.</font><br>
